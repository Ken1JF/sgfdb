...
func ExampleReadWriteDatabase() {
	const second_dir = "../sgf"
	var useDir = ""
	_, err := os.Stat(gogod_dir)
	if err != nil {
		fmt.Println("Not doing GoGoD test, error:", err, "accessing:", gogod_dir)
	...
	// Output:
	// Not doing GoGoD test, error: stat /usr/local/GoGoD: no such file or directory accessing: /usr/local/GoGoD
